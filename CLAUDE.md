...
- **M5 ratings: SPECCED, not built.** Decisions made with Suryansh: /100 display, Elo internal (start 1000, K 40→24 at 10 matches, margin multiplier cap 1.5, logistic display mapping), full-history backfill; plus trust hardening (log_match consent guard incl. tournament-mates, stats shape guard). Spec: `docs/superpowers/specs/2026-07-14-ratings-design.md`. **Next action: write the implementation plan, then execute on branch `m5-ratings`.** Session handoff with process detail + environment quirks: `docs/superpowers/handoffs/2026-07-14-m5-ratings-handoff.md`.
