...
- **v1 findings closed by M2:** orphaned partial inserts (atomic RPC), outcome spoofing (server-derived), friendship immutability, reverse-duplicate friendships. **Still open (minor):** friends-screen `accept` mutation has no onError; step counter shows "of 5" even when the format step is skipped; no "add a friend" hint on the FFA roster step; template cruft (unused deps/assets, splash color) parked for the design milestone.
...
- **Roadmap:** M3 live scoring (sport-native, scorer role, Supabase Realtime) → M4 tournament mode (round robin + knockout, fixtures server-side). Each starts with its own spec. Ledger: `.superpowers/sdd/progress.md` (git-ignored).
